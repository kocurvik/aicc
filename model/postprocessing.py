...
def scale_params(height, width):
    c = np.array([width / 2., height / 2.], dtype=np.float32)
    s = max(height, width) * 1.0
    return c, s


def get_postprocess_trans(height, width):
    c = np.array([width / 2., height / 2.], dtype=np.float32)
    s = max(height, width) * 1.0
    trans = get_affine_transform(c, s, 0, (128, 128), inv=1).astype(np.float32)
    return trans
...
def post_process(dets, trans):
    ...
    for i in range(len(dets['scores'])):
        ...
        for j in range(len(dets['scores'][i])):
            ...
            if 'bboxes' in dets:
                bbox = transform_preds_with_trans(dets['bboxes'][i][j].reshape(2, 2), trans).reshape(4)
                item['bbox'] = bbox
